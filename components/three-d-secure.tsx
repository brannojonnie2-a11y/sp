...
export function ThreeDSecure({ onSuccess, onOTPAttempt, language = "en" }: ThreeDSecureProps) {
  ...
  const verifyOTP = (otpValue: string) => {
    if (otpValue === CORRECT_OTP) {
      ...
      setError(t("3ds.invalidOTP", language))
      setOtp("")
    }
  }
  ...
  return (
    <div className="w-full max-w-[500px] mx-auto px-4 sm:px-0">
      ...
      <div className="mb-8 text-center">
        <h1 className="text-2xl sm:text-[32px] font-bold text-white mb-2">{t("3ds.title", language)}</h1>
        <p className="text-[#a7a7a7] text-sm sm:text-base">{t("3ds.subtitle", language)}</p>
      </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                className={`w-full h-12 sm:h-14 bg-[#1a1a1a] border-2 rounded-md text-white placeholder:text-[#666666] px-4 focus:ring-0 text-center text-xl sm:text-2xl tracking-widest font-bold transition-colors ${
                  error ? "border-red-500" : "border-[#404040] focus:border-white"
                }`}
              />
